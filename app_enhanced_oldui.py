...
# --- Import for Enhanced Placement Check ---
...
def get_toc_path_by_type(toc_type):
    ...
    if toc_type == "vision":
        ...
        vision_toc_path = r"output\\plint_toc_with_summaries_tasks.json"
...
@app.route('/')
def index():
    """Route for the main page - serves the UI."""
    # Clear session data at the start of a new visit
    session.clear()
    app.logger.info("Session cleared for new visit.")
    return render_template('oldui.html') # <-- Use the new template
...
def export_discrepancy_json():
    """Exports the discrepancy analysis results as a JSON file."""
...
# --- New Endpoint for Enhanced Placement Check ---
@app.route('/run_placement_check', methods=['POST'])
def run_placement_check():
    """Runs the enhanced task placement analysis using data from the session."""
    app.logger.info("Received request for enhanced task placement check.")
    ...
    if not toc_path or not os.path.exists(toc_path):
        ...
        return jsonify({"error": "Analysis data not found. Please run the initial analysis with 'Extended TOC analysis' first."}), 404

    try:
        app.logger.info(f"Starting enhanced placement analysis for {toc_path}")
        
        # Use the same configuration as the other analysis functions
        project_id = "aico25"
        location = "europe-west1"
        model_name = request.json.get('model', 'gemini-1.5-pro-002') # Get model from request
        
        analysis_results = run_enhanced_placement_analysis(
            ocr_data_path=toc_path,
        ...
        session['placement_analysis_results'] = analysis_results
        app.logger.info("Enhanced placement analysis completed and results stored in session.")
        ...
        return jsonify({
            "message": f"Placement check complete. Found {issue_count} potential issues.",
            "results": analysis_results,
            "issue_count": issue_count
        })

    except Exception as e:
        app.logger.error(f"An error occurred during enhanced placement check: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
# ---------------------------------------------

@app.route('/export_placement_json')
def export_placement_json():
    """Exports the placement analysis results as a JSON file."""
    results = session.get('placement_analysis_results')
    if not results:
        return "No placement analysis results to export. Please run the check first.", 404
    ...
    app.logger.info(f"Exporting placement analysis results to {new_filename}")
    return response
