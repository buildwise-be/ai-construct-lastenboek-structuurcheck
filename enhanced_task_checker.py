...
class EnhancedTaskChecker:
    ...
    def _generate_enhanced_summary(
        ...
        if any(
            ...
            for word in ["material", "equipment", "tool", "materiaal"]
        ):
            execution_details.append("Specifies materials or equipment")

        if any(
            word in content_lower
            for word in ["method", "procedure", "process", "methode"]
        ):
            execution_details.append("Describes execution methods")

        if any(
            word in content_lower
            for word in ["quality", "standard", "specification", "kwaliteit"]
        ):
    ...
    def _identify_issues(
        ...
        if title_indicators and content_type not in title_indicators:
            issues.append(
                f"Content type ({content_type}) may not match title indicators"
            )
        ...
        if content_type in ["electrical", "plumbing", "hvac"] and not any(
            word in content_lower for word in ["install", "connect", "pipe", "wire"]
        ):
    ...
    def _suggest_improvements(
        self, content: str, title: str, content_type: str, issues: List[str]
    ) -> List[str]:
        ...
        if "Content type" in str(issues):
            suggestions.append(
                "Review section placement or adjust content to match title"
            )
    ...
    def _analyze_batch_with_llm(
        self, batch: List[Dict], model: GenerativeModel
    ) -> Dict[str, Any]:
        """
        ...
        prompt_parts = [
            ...
            (
                "Evaluate if the tasks described seem contextually appropriate for the section's title "
                "and its place in the hierarchy, using the ideal structure as a reference."
            ),
            (
                "Be flexible; the document being analyzed may not match the ideal structure perfectly, "
                "but the conceptual grouping of tasks should be logical."
            ),
            "",
        ...
        for chapter in self.chapters:
            ...
            analysis = EnhancedSectionAnalysis(
                ...
                summary=llm_result.get(
                    "summary", "Analysis may have failed for this item."
                ),
                section_id=section_id,
                content_type="llm_analyzed",  # We can use a new content type
                confidence=llm_result.get(
                    "confidence", 0.9 if "issues_found" in llm_result else 0.5
                ),
    ...
    def generate_summary_format_report(
        self, analyses: List[EnhancedSectionAnalysis]
    ) -> List[Dict]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Enhanced Task Placement Checker - Summary Format"
    )
    parser.add_argument(
        "--ocr-data",
        default=(
            "ocroutput/pipeline_run_20250605_112516_cathlabarchitectlb/final_combined_output/"
...
def analyze_placement_from_file(
    ocr_data_path: str,
    project_id: str,
    location: str,
    model_name: str,
    ...
    try:
        ...
        if verbose:
            print(
                f"Initializing Vertex AI with project={project_id}, location={location}"
            )
        ...
        if verbose:
            print(
                f"Analyzing {len(checker.chapters)} sections with model {model_name}..."
            )
        ...
        if verbose:
            print(
                f"Enhanced placement analysis completed. Analyzed {len(result)} sections."
            )
