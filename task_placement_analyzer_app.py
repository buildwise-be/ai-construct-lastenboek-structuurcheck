...
def get_latest_enhanced_toc_path():
    ...
    try:
        ...
        if not found_files:
            app.logger.error(f"No enhanced TOC file ('chapters_with_text_v3.json') found using pattern: {search_pattern}")
            return None
...
def run_placement_analysis(json_path):
    ...
    try:
        ...
        app.logger.info(f"Enhanced placement analysis completed.")
...
def analyze():
    ...
    toc_path = os.path.join('ocroutput', selected_file_id, 'final_combined_output', 'chapters_with_text_v3.json')

    if not os.path.exists(toc_path):
        app.logger.error(f"Selected analysis file does not exist at path: {toc_path}")
    ...
    try:
        ...
        return jsonify({
            "message": f"Analyse voltooid. {issue_count} mogelijke problemen gevonden.",
            "status": "complete",
...
def get_enhanced_results():
    ...
    if results:
        ...
        app.logger.info(f"Returning enhanced results.")
